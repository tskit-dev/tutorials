...
In the {ref}`previous tutorial<sec_tskit_forward_simulations>`, we developed the following
basic forward-time Wright-Fisher (WF) simulator (refer back to that tutorial for a
detailed run through of the code):
...
def initialise_pop(tables, time, size) -> dict:
    return dict(make_diploid(tables, time) for _ in range(size))

def simple_diploid_sim(diploid_pop_size, generations) -> tskit.TreeSequence:
...
We will now implement the WF simulation above but with simplification happening at
...
def simplify_tables(tables, samples, pop) -> dict[int, tuple[int, int]]:
    ...
    population dict in which individual and nodes are remapped.
    """
    tables.sort()
...
`tskit`'s sorting requirements. Therefore, the results of our `diploid_sim()` function
...
ts = diploid_sim(10, 500, simplification_interval=1)
...
for i in test_intervals:
    ...
    random = np.random.default_rng(42)
    ts_test = diploid_sim(10, 500, simplification_interval=i)
