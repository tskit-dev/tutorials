...
{class}`TreeSequence` object; what now? This tutorial is aimed at
...
ts = msprime.sim_ancestry(
    ...
    random_seed=1234,  # only needed for repeatability
    )
...
Although tskit is designed so that it is rapid to pass through trees sequentially,
...
{program}`tskit` also provides efficient ways to return {class}`Site` and
...
full variation data - allelic states and node genotypes at each site.
Since nodes are by definition haploid, their genotype at a site is simply the
allelic state, but for efficiency, the {attr}`~Variant.genotypes` attribute
...
adjacent site, and so on along the genome. It is much less efficient to look at all the
...
a single sample can be obtained via the inefficient {meth}`TreeSequence.haplotypes` method.
If a reference sequence is defined, 
:::
...
for v in ts.variants():
    site = v.site
    alleles = np.array(v.alleles)  # NB, a shortcut is to use v.states()
...
windows = np.linspace(0, L, num=L//10_000 + 1)
...
on randomly occurring mutations at sites.
...
with genotype data for most users' needs. For example, it gives us an
another way to compute the pairwise diversity statistic:
...
Here's the equivalent using the native {meth}`TreeSequence.diversity` method,
{ref}`windowed <tskit:sec_stats_windows>` by site:
...
Note, however, that you would normally want to set `span_normalise=True`  to account
for the fact that sites are not evenly spread along the genome (see {ref}`these docs<tskit:sec_stats_span_normalise>`).

(sec_tskit_getting_started_key_points)=

## Key points covered above

Here are the simple methods and take-home messages from this introduction to the
